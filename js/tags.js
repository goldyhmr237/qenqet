...
triggers: {
'@': { minChars: 1, uniqueTags: false, showImageOrIcon: true,keys_map:{
	id: 'id',
	name: 'name',
...
classes: {
	...
	var trigger_defaults = {
		...
		keys_map: { id: 'id', title: 'name', description: '', img: 'img', no_img_class: 'icon', type: 'type' },
		syntax: _.template('@[<%= type %>:<%= id %>:<%= title %>]'),
		parser: /(@)\[([\w\s\.\-]+):(\d+):([\w\s@\.,-\/#!$%\^&\*;:{}=\-_`~()]+)\]/gi,
		parserGroups: {type: 2, id: 3, title: 4},
		classes: {
	...
	var utils = {
		...
		highlightTerm: function(value, term) {
			...
			return "<b>"+value+" <i class='icicon-arrow-circle-o-left'></i></b>";
		},
	...
	var TextNTags = function(editor) {
		...
		return {
			...
			getPlainText: function(callback) {
				if(!_.isFunction(callback)) {
					return;
				}
				callback.call(this, parseTaggedText(getTaggedText()).plain_text);
			},
